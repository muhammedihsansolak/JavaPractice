package coreJavaPractice.tjmaxx;
...
public class TJMaxx {
    ...
    public double getItemPrice(int catalogNumber)
    {
        ...
        list.addAll(onSaleItems);
        for (Item each : list) {
            if (each.getCatalogNumber()==catalogNumber){ return each.getPrice();}else return 0.0;
        }
        return 0;
    }
    ...
    public void removeItem(int catalogNumber) {
        ...
        list.addAll(regularItems);
        list.addAll(onSaleItems);
        list.removeIf(p -> p.getCatalogNumber()==catalogNumber);
    }
    ...
    public void buyItem(int catalogNumber) {
        ...
        for (OnSaleItem each : onSaleItems) {
            if (each.getCatalogNumber()==catalogNumber && onSaleItemsCount()>0){
